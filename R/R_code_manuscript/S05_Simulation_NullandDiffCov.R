...
setwd("/pine/scr/m/e/meichen/hovestdta/data/TCAI/E0022")
...
if(NCHANGE ==1){
  if ("scenario" == "diffstructure"){
    ...
    count1 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample*2)
    ...
    # count1 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 20, n = nsample)
    y1 = count1$count[1:nsample,]
    y2 = count1$count[(nsample + 1):(nsample*2),]
    
    # assign slightly diff mean to y2
    diffm = sample(1:200, xdiffpct/100 * 200)
    for(i in diffm){
      y2[,i] = y2[,i] + runif(1, 2,5)
    }
    y = rbind(y1, y2)
...
if(NCHANGE ==2){
  if ("scenario" == "diffstructure"){
    ...
    count1 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample)
    count2 <- CountMap6(sigma = sigma.list.1[[2]], ngene = 200, n = nsample, scmeanvec = count1$thetaj, pijvec = count1$pij)
    count3 <- CountMap6(sigma = sigma.list.1[[1]], ngene = 200, n = nsample, scmeanvec = count1$thetaj, pijvec = count1$pij)
  } else if ("scenario" == "samestructure"){
    # same structure, different mean
    nivec.list <- list(nivec= c(100,100),nivec2 = c(100,100))
    sigma.list.1 <- generateSigmaList(nivec.list = nivec.list, structure = "Identical S, Diff W", diffblk = NULL)
    # count1 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample)
    # count2 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample)
    # count3 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample)
    # count2 <- CountMap6(sigma = sigma.list.1[[1]], ngene = 200, n = nsample, scmeanvec = count1$thetaj, pijvec = count1$pij, meanDiffPct = 0.2)
    # count3 <- CountMap6(sigma = sigma.list.1[[1]], ngene = 200, n = nsample, scmeanvec = count1$thetaj, pijvec = count1$pij, meanDiffPct = 0.2)
    
    count1 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample*3)
    # count2 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample)
    # count2 <- CountMap6(sigma = sigma.list.1[[1]], ngene = 200, n = nsample, scmeanvec = count1$thetaj, pijvec = count1$pij, meanDiffPct = 0.2)
    # count1 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 20, n = nsample)
    y1 = count1$count[1:nsample,]
    y2 = count1$count[(nsample + 1):(nsample*2),] 
    y3 = count1$count[(nsample*2 + 1):(nsample*3),]
    ...
    # count3 <- CountMap5(sigma = sigma.list.1[[1]], ngene = 200, n = nsample)
    count2 <- CountMap6(sigma = sigma.list.1[[2]], ngene = 200, n = nsample, scmeanvec = count1$thetaj, pijvec = count1$pij, meanDiffPct = xdiffpct/100)
    count3 <- CountMap6(sigma = sigma.list.1[[1]], ngene = 200, n = nsample, scmeanvec = count1$thetaj, pijvec = count1$pij, meanDiffPct = xdiffpct/100)
...
if(simnum == 1 & NCHANGE == 1 & "scenario" == "samestructure" & nsample == 200){
  save.image("E0014_test.rda")
}

write.table(sumtable, "E0022_CP_NCHANGE_SIM_simnum_NSAMnsample_scenario_xdiffpct_yform_covcpd.txt")
